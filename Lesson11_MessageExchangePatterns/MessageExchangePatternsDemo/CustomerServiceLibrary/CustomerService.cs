...
namespace CustomerServiceLibrary
{
    ...
    public class CustomerService : ICustomerService
    {
                    ...
                    try
                    {
                        using (SqlDataReader CustomersReader = cmd.ExecuteReader())
                        {
                            while (CustomersReader.Read())
                            {
                                ...
                                customer.customerId = CustomersReader.GetInt32(0);//Error made to throw exception in this line
        ...
        public void LogIn(string user)
        {
            ...
            writeString.WriteLine("{0} started the application at {1:t}", user, DateTime.Now);
            File.AppendAllText(@"C:\Users\Yeswanth\Desktop\SourceInfotech\WCF\Programs\Lesson11_MessageExchangePatterns\UserLog.txt", writeString.ToString());
        }

        public void LogOut(string user)
        {
            var writeString = new StringWriter();
            writeString.WriteLine("{0} closed the application at {1:t}", user, DateTime.Now);
            File.AppendAllText(@"C:\Users\Yeswanth\Desktop\SourceInfotech\WCF\Programs\Lesson11_MessageExchangePatterns\UserLog.txt", writeString.ToString());
        }
    }
}
